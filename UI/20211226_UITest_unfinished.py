...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        command = lambda : goToScan(), padx=10, pady=10)
...
class HistoryLogPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        MoodIndex = ['Happiness', 'Energy', 'Test Index']
        historyDict
        ...
        *list([changeDateToDateText(date) for date in sorted(historyDict.keys(),reverse=True)]))
        ...
        self.TimeListButton = tk.Button(self, text ="Name", font=SMALLFONT)
        self.TimeListButton.grid(row = 6, column = 2, padx = 0, pady = 0, sticky="ew")
        self.MoodIndexListButton = tk.Button(self, text ="Type", font=SMALLFONT)
        self.MoodIndexListButton.grid(row = 6, column = 3, padx = 0, pady = 0, sticky="ew")
        self.ActivityPickedListButton = tk.Button(self, text ="Like Status", font=SMALLFONT)
        ...
        def createDataForGraph(historyDictionary, configuration):
            ...
            for timeName in historyDictionary[changeDateTextToDate(configuration[1])].keys():
                time = createTimeFromTimeName(timeName)
                # print(time)
        ...
        def createTimeFromTimeName(time):
            time = time.split('.')
    ...
    def addDataToHistoryListbox(self,data):
        ...
        for i in range(len(data[0])):
            self.TimeList.insert(END, data[0][i])
            self.MoodIndexList.insert(END, data[1][i])
...
for i in range(16):
    historyDict['20211213'][f"{i+5:02}"+'.30'] = [((i^2-1+random.randint(0,5))%16)/16, DatabaseActivityList[random.randint(0,49)][0] ]
    historyDict['20211214'][f"{i+5:02}"+'.30'] = [((i^2+i+random.randint(0,5))%16)/16, DatabaseActivityList[random.randint(0,49)][0] ]
    historyDict['20211215'][f"{i+5:02}"+'.30'] = [(i^2+random.randint(0,5))%16/16, DatabaseActivityList[random.randint(0,49)][0] ]
    historyDict['20211216'][f"{i+5:02}"+'.30'] = [((i^2+1+random.randint(0,5))%16)/16, DatabaseActivityList[random.randint(0,49)][0] ]
#print(historyDict)
